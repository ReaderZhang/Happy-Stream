...
func RegisterUserHTTPServer(s *http.Server, srv UserHTTPServer) {
	...
	r.GET("/v1/member/list/get/time/{start}/{end}/{page_num}/{page_size}", _User_ListMemberGetBetweenTime0_HTTP_Handler(srv))
	r.POST("/v1/member/list", _User_ListMembers0_HTTP_Handler(srv))
...
func _User_ListMembers0_HTTP_Handler(srv UserHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in MemberForm
		if err := ctx.Bind(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, "/user.v1.User/ListMembers")
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.ListMembers(ctx, req.(*MemberForm))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
		}
		reply := out.(*MemberTableReply)
		return ctx.Result(200, reply)
	}
}
...
type UserHTTPClient interface {
	...
	ListMembers(ctx context.Context, req *MemberForm, opts ...http.CallOption) (rsp *MemberTableReply, err error)
...
func (c *UserHTTPClientImpl) ListMemberGetBetweenTime(ctx context.Context, in *ListMemberGetBetweenTimeReq, opts ...http.CallOption) (*ListMemberGetBetweenTimeReply, error) {
	...
	pattern := "/v1/member/list/get/time/{start}/{end}/{page_num}/{page_size}"
	path := binding.EncodeURL(pattern, in, true)
...
func (c *UserHTTPClientImpl) ListMembers(ctx context.Context, in *MemberForm, opts ...http.CallOption) (*MemberTableReply, error) {
	var out MemberTableReply
	pattern := "/v1/member/list"
	path := binding.EncodeURL(pattern, in, false)
	opts = append(opts, http.Operation("/user.v1.User/ListMembers"))
	opts = append(opts, http.PathTemplate(pattern))
